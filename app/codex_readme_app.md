...
- Вызов tool маршрутизируется по `preferredServerId + toolName`; при неоднозначности возвращается ошибка/clarification.
  Как лучше это показать на 10 вопросах:


Проверка 1 — сильные вопросы

Вопросы, где ответ точно есть в базе.
Смотришь:
•	есть ли sources
•	есть ли quotes
•	совпадают ли цитаты с ответом по смыслу

Проверка 2 — слабые / нерелевантные вопросы

Пара вопросов, которых нет в базе.
Смотришь:
•	ответил ли ассистент «не знаю»
•	не выдумал ли источники
•	не показал ли фейковые цитаты

Проверка 3 — вопросы с ожидаемыми источниками

Сверяешь expectedSources с тем, что реально отобразилось.
